...
PLUGINLIB_EXPORT_CLASS(laser_filters::LaserScanMultiBoxFilter,
...
bool laser_filters::LaserScanMultiBoxFilter::update(
    ...
    bool success = tf_.waitForTransform(
        box_frame_, input_scan.header.frame_id,
        input_scan.header.stamp +
            ros::Duration().fromSec(input_scan.ranges.size() *
                                    input_scan.time_increment),
        ros::Duration(1.0), ros::Duration(0.01), &error_msg);
    if (!success) {
        ROS_WARN("Could not get transform, irgnoring laser scan! %s",
                error_msg.c_str());
        return false;
    }
    // polar to cartesian
    try {
        projector_.transformLaserScanToPointCloud(box_frame_, input_scan,
                                                laser_cloud, tf_);
    ...
    const int i_idx_c =
        sensor_msgs::getPointCloud2FieldIndex(laser_cloud, "index");
    ...
    if (i_idx_c == -1 || x_idx_c == -1 || y_idx_c == -1 || z_idx_c == -1) {
        ROS_INFO_THROTTLE(.3,
                        "x, y, z and index fields are required, skipping scan");
    }
    ...
    for (i_idx = i_idx_offset, x_idx = x_idx_offset, y_idx = y_idx_offset,
        z_idx = z_idx_offset;
        ...
        if (box_[i].is_in(x, y)) {
            // 2022.01.27 : jglee
            // obstacle_layer 에서 cart_leg 를 지워줘야 하기 때문에, inf data 로
            // clear 함(Nan -> inf).
